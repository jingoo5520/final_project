...
public class BannerServiceImpl implements BannerService {
    ...
    public void uploadBanner(MultipartFile file) {
        if (!file.isEmpty()) {
            ...
            // 배너 파일 저장 경로
            File destinationFile = new File(bannerPath + fileName);
            try {
                // 파일 저장
                ...
                banner.setFilePath(bannerPath + fileName); // 전체 경로 설정
                // DB에 배너 정보 저장
    ...
    @Override
    public List<BannerVO> getAvailableBanners() {
        return bDao.getBanners(); // 배너 목록 조회
    }

    @Override
    public void setSelectedBanner(String fileName) {
        // 선택된 배너를 DB나 설정 파일에 저장하는 로직
        bDao.updateSelectedBanner(fileName);
    }
}
