...
public class UserProductServiceImpl implements ProductService {
	...
	public int saveProduct(ProductDTO productDTO, List<String> list) {
		...
		for (String imageUrl : list) {
			if (imageUrl.contains("Main_")) {
				mainImage = imageUrl; // "main"?ù¥ ?è¨?ï®?êú ?ù¥ÎØ∏Ï??äî Î©îÏù∏ ?ù¥ÎØ∏Ï?Î°? Ï≤òÎ¶¨
			} else if (imageUrl.contains("Sub_")) {
				subImages.add(imageUrl); // "sub"?ù¥ ?è¨?ï®?êú ?ù¥ÎØ∏Ï??äî ?ÑúÎ∏? ?ù¥ÎØ∏Ï?Î°? Ï≤òÎ¶¨
			}
		}
		if (mainImage != null) {
	...
	private PagingInfo makePagingInfo(ProductPagingInfoDTO dto, Map<String, Object> map) {
		...
		pi.setTotalPageCnt(); // ?†ÑÏ≤? ?éò?ù¥Ïß? ?àò ?Ñ∏?åÖ
		pi.setStartRowIndex(); // ?òÑ?û¨ ?éò?ù¥Ïß??óê?Ñú Î≥¥Ïó¨Ï£ºÍ∏∞ ?ãú?ûë?ï† Í∏??ùò indexÎ≤àÌò∏
