...
window.onload = function() {
	...
	let data = {
 	...
 	$.ajax({
		type : "POST",
		url: window.location.origin + "/user/kakaopay_payRequest", // 절대 경로
		data : JSON.stringify(data),
		contentType : "application/json",
		success : function(response) {
			console.log("결제요청의 결과, response : " + response)
			if (response.result == "success") {
			    ...
			    if (window.opener) {
			        ...
			        // TODO : 실제 결제완료 페이지로 이동시키기
			       	window.opener.location.href = window.location.origin + "/user/temp_01"
			    }
				window.close();
			} else {
